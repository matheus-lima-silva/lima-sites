...
def check_whatsapp_credentials():
    ...
    if not settings.whatsapp_configured:
        ...
        error_msg = (
            f'Credenciais do WhatsApp não configuradas: {", ".join(missing)}'  # noqa: E501
        )
...
async def verify_webhook_signature(
    request_body: bytes, signature_header: str
) -> bool:
    """
    ...
    try:
        ...
        signature = hmac.new(
            key, msg=request_body, digestmod=hashlib.sha256
        ).hexdigest()
...
async def send_text_message(to: str, message: str) -> Dict[str, Any]:
    ...
    Raises:
        WhatsAppError:
          Se as credenciais do WhatsApp não estiverem configuradas.
    """
    ...
    try:
        ...
        if message_type == 'text':
            ...
            if 'button_reply' in interactive:
                result['message_content'] = interactive['button_reply'].get(
                    'id'
                )  # noqa: E501
            elif 'list_reply' in interactive:
                result['message_content'] = interactive['list_reply'].get('id')  # noqa: E501
...
async def send_interactive_message(
    ...
    Args:
        ...
        buttons:
        Lista de botões no formato
          [{"id": "btn_id", "title": "Texto do botão"}]
