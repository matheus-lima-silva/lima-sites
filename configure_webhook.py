...
def main():
    ...
    if "WHATSAPP_WEBHOOK_URL" in status["recomendadas_ausentes"]:
        url_base = input("Digite a URL base da sua aplicação (ex: https://seuapp.com): ")
        if url_base:
    ...
    if not status["ausentes"]:
        ...
        print("2. Selecione seu aplicativo WhatsApp")
        print("3. No menu lateral, vá para 'WhatsApp > Configuração'")
        print("4. Em 'Webhooks', clique em 'Editar'")
        print("5. Configure com os seguintes valores:")
        print(f"   - URL do webhook: {NEGRITO}{webhook_url}{RESET}")
        print(f"   - Token de verificação: {NEGRITO}{verify_token}{RESET}")
        print("   - Campos de assinatura: mensagens (pelo menos)")
        print("6. Clique em 'Verificar e salvar'")
    ...
    if not status["ausentes"]:
        ...
        if test_local.lower() == 's':
            ...
            print("Para testar usando ngrok, abra outro terminal e execute:")
            print("  ngrok http 8000")
            print("Use a URL fornecida pelo ngrok + '/auth/whatsapp/webhook' no painel do WhatsApp")
